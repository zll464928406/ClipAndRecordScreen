...
+ (void)removeCircleViewFromView:(NSView *)view;
...
+ (CircleCounterView *)showCircleDownWithSeconds:(float)seconds onView:(NSView *)view withSize:(CGSize)size andType:(CircleDownCounterType)type {
    ...
    CircleCounterView *circleView = [[CircleCounterView alloc] initWithFrame:[self frameOfCircleViewOfSize:size inView:view]];
    ...
    switch (type) {
        ...
        case CircleDownCounterTypeOneDecimalDecre:
            circleView.numberFont = [NSFont fontWithName:@"Courier-Bold" size:15.0f];
